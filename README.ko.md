...
* **네트워크 불안정으로 인한 중복 요청 및 정합성 꼬임:** 모바일 환경에서 인터넷 접속이 순간적으로 단절되거나 느릴 때, 유저의 다중 터치(더블 탭) 혹은 클라이언트의 네트워크 재시도로 인해 동일한 예약이 중복 생성되거나 최초 신청 시간보다 늦게 등록되어 순서 정합성이 왜곡될 위험이 있습니다.
...
* **클라이언트 기반 고유 식별자 및 멱등성(Idempotency) 검증 설계:** 클라이언트가 신청을 누른 시점에 시간 정보와 난수를 조합한 고유 ID(`waiting_id`)와 등록 시각(`registration_time`)을 직접 생성해 전송하고, 서버가 이를 DB에서 대조하여 중복인 경우 추가 생성 없이 기존 도큐먼트를 멱등하게 반환함으로써 중복 등록을 원천 차단했습니다.
...
* **네트워크 지연과 무관한 등록 시간 보존:** 서버에 요청이 실제 인입되는 시점 대신, 유저가 클라이언트 화면에서 실제 "신청" 버튼을 누른 최초 시각을 서버가 덮어쓰지 않고 보존하게끔 처리하여 대기 순열의 시간 정합성이 왜곡되는 것을 방지했습니다.
