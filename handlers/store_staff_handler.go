...
// JoinStoreHandler handles the request for a staff to join a store
func JoinStoreHandler(w http.ResponseWriter, r *http.Request) {
	...
	// 5. Check if already joined/pending
...
// GetStoreStaffHandler handles the request to get all staff for a store
func GetStoreStaffHandler(w http.ResponseWriter, r *http.Request) {
	...
	// 1. Verify Token & Permission (Manager only)
	authHeader := r.Header.Get("Authorization")
	if authHeader == "" {
		utils.RespondWithError(w, "Authorization header is required", http.StatusUnauthorized)
		return
	...
	// Check if user is manager of this store
...
// UpdateStoreStaffStatusHandler handles the request to update a staff member's status
