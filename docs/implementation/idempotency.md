...
### ①が効く条件 (2026-09-18 時点の実態)

**キーが「論理的な登録1件」に対して1つでなければ、この層は何もしない。**
同じ登録の再試行で毎回新しいキーを作ると、サーバーから見れば別々の要求であり、
②③をすり抜けて普通に2件目が作られる。

現状のクライアントは**送信ボタンを押すたびに新しいキーを生成**している。

```js
// yoyaku_mate/src/containers/waiting-screen/WaitingScreenContext.jsx
const handleSubmitWaiting = useCallback(async () => {
    const { waitingId: clientWaitingId } = getJSTDateStrings();  // ← 押下ごとに新規
```

点主アプリも同じ形 (`waiting_providers.dart` の `addWaitingItem` 内で生成)。
加えて**両クライアントとも自動リトライを持たない** (axios-retry もインターセプタも無い)。

したがって、現在①が実際に効くのは次のケースに限られる。

| ケース | ①が効くか |
|---|---|
| 混雑ポップアップの確認ボタンの二度押し | **効く。** `pendingPayload` に payload ごと保持されるため同じキーが2回飛ぶ |
| 通信失敗 → 客が登録ボタンを押し直す | **効かない。** 新しいキーになるため2件目が作られる |
| 自動リトライによる再送 | 該当なし (実装されていない) |

つまり**ドキュメントが当初うたっていた「ネットワーク再試行時の二重登録防止」は、
サーバー側だけでは達成されていない。** 達成するにはクライアント側で、キーを
「押下ごと」ではなく「登録1件ごと」に発行し、成功するまで再利用する必要がある。

②③にはそれとは独立した価値がある (サーバー生成IDの衝突防止、二度押しの遮断、
そして「(store_id, waiting_id) は一意」という不変式が今後どんな経路が増えても
崩れないこと) ため、①の状態に関わらず入れてある。
