...
> **서버만으로는 완성되지 않는다.** ②③은 서버 안에서 닫히지만, ①은 클라이언트가
> "같은 등록의 재시도에서는 같은 키를 보낸다"는 것을 지켜야 비로소 성립한다.
> 현재 클라이언트 구현은 아래 "①이 실제로 동작하는 조건" 참조.
...
### ①이 실제로 동작하는 조건 (2026-09-18 시점 실태)

**키가 "논리적 등록 1건"에 하나가 아니면 이 층은 아무 일도 하지 않는다.**
같은 등록의 재시도에서 매번 새 키를 만들면 서버 입장에서는 서로 다른 요청이고,
②③을 그냥 통과해 2건째가 생성된다.

현재 클라이언트는 **전송 버튼을 누를 때마다 새 키를 생성**한다.

```js
// yoyaku_mate/src/containers/waiting-screen/WaitingScreenContext.jsx
const handleSubmitWaiting = useCallback(async () => {
    const { waitingId: clientWaitingId } = getJSTDateStrings();  // ← 누를 때마다 새로
```

점주 앱도 같은 형태다 (`waiting_providers.dart`의 `addWaitingItem` 안에서 생성).
게다가 **두 클라이언트 모두 자동 재시도가 없다** (axios-retry도 인터셉터도 없음).

따라서 현재 ①이 실제로 효과가 있는 경우는 다음으로 한정된다.

| 케이스 | ①이 듣는가 |
|---|---|
| 혼잡 팝업 확인 버튼 두 번 누름 | **듣는다.** `pendingPayload`에 payload째 보관되어 같은 키가 2번 날아감 |
| 통신 실패 → 손님이 등록 버튼을 다시 누름 | **안 듣는다.** 새 키가 되어 2건째가 생성됨 |
| 자동 재시도에 의한 재전송 | 해당 없음 (구현되어 있지 않음) |

즉 **문서가 당초 내걸었던 "네트워크 재시도 시 이중 등록 방지"는 서버만으로는
달성되지 않았다.** 달성하려면 클라이언트에서 키를 "누를 때마다"가 아니라
"등록 1건마다" 발행하고, 성공할 때까지 재사용해야 한다.

②③에는 그와 무관한 값이 있다 (서버 생성 ID의 충돌 방지, 두 번 누름 차단,
그리고 "(store_id, waiting_id)는 유일"이라는 불변식이 앞으로 어떤 경로가 늘어나도
깨지지 않는 것). 그래서 ①의 상태와 무관하게 넣어두었다.
