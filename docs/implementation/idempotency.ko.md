...
**2026-09-18까지 두 클라이언트 모두 전송 버튼을 누를 때마다 새 키를 만들고 있었다.**
게다가 자동 재시도가 없어서(axios-retry도 인터셉터도 없음) "재시도"란 곧
손님/직원이 버튼을 다시 누르는 것이었고, 그때마다 키가 바뀌었다.
즉 이 문서가 내걸었던 "네트워크 재시도 시 이중 등록 방지"는 **서버에 ②③을 넣은
뒤에도 여전히 달성되지 않은 상태였다.**

같은 날 두 클라이언트를 수정해 키를 "등록 1건마다" 발행하고 성공할 때까지
재사용하도록 했다. 상세는 각 저장소를 참조:

- `yoyaku_mate/docs/implementation/idempotency.ko.md` (고객 웹)
- `yoyaku_mate_provider/docs/implementation/idempotency.ko.md` (점주 앱)

점주 앱은 키와 함께 **등록 내용의 지문**을 들고 다닌다. 키만 재사용하면
손님 A 등록 실패 후 손님 B를 등록할 때 A의 키가 쓰여, A가 실은 성공해 있었다면
B 대신 A의 레코드가 반환된다. 매장에서는 손님을 연달아 등록하므로 실제로 일어난다.

> **교훈:** ②③은 서버 안에서 닫히므로 "구현했다"고 말하기 쉽지만, ①이 없으면
> 전체로서는 아무것도 막지 못한다. 서버만 보고 완료로 판단하지 말 것.

②③에는 ①과 무관한 값도 있다 — 서버 생성 ID의 충돌 방지, 두 번 누름 차단,
