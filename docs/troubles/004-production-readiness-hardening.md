...
- ~~**シークレットのコミット**: 履歴上も無し~~
  → **この判断は誤りだった。** 2026-09-18に [006](./006-committed-credential-in-history.md) で、
  `config.development.json` に実際のAtlas接続文字列 (ユーザー名・パスワードを含む) が
  入ったままコミットされており、履歴51コミットに残っていることを確認した。
  当時の点検は**現時点のファイルだけを見て**履歴を見ていなかった。
  削除済み (`379d149`) のファイルは `git ls-files` にも `git grep` にも現れない
