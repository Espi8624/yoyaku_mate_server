...
향후 "버튼 클릭 한 번으로 시프트표 자동 생성" 기능의 기반 데이터로, 매장 스태프 본인이 요일별로 근무 가능한 시간대(오전·오후)를 입력하고, 매장 매니저가 이를 조회·수정할 수 있는 기능입니다.
...
3. **시간대 단위**:
   - `MORNING`(오전) / `AFTERNOON`(오후) 2구분 중, 요일별로 복수 선택이 가능합니다.
...
* 각 요일 필드의 값은 `MORNING` / `AFTERNOON` 중 하나로 이루어진 문자열 리스트입니다.
