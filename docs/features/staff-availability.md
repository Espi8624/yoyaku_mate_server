...
将来的な「ボタン一つでのシフト表自動生成」機能の土台となるデータとして、店舗スタッフ本人が曜日ごとに勤務可能な時間帯(午前・午後)を入力し、店舗マネージャーがそれを照会・修正できる機能です。
...
3. **時間帯の単位**:
   - `MORNING` (午前) / `AFTERNOON` (午後) の2区分から、曜日ごとに複数選択可能です。
...
* 各曜日フィールドの値は `MORNING` / `AFTERNOON` のいずれかの文字列リスト。
